...
| C11 | **Auto-training del ensemble por warm-up (onboarding operativo)** | ✅ HECHO backend + frontend (2026-07-16); E2E TALARA pendiente | Cierra el ciclo `CR310 → ingesta → warm-up automático → θ propio → monitoreo activo` sin intervención manual. Migración `ml_007` + tabla `station_training_state` (máquina `nueva→recolectando→entrenando→entrenado`/`error`) con seed de las 5 estaciones vigentes como `entrenado` (CA-15). `training_service.train_station(source={warmup,retrain})` reusa el pipeline (StandardScaler + MLP(3,) + IF(200, 0.05) + θ=P95), escritura atómica `.tmp+os.replace`, guarda CR-04 para retrain. Trigger en `health_service.evaluate()` post-commit con UPDATE conditional anti-race + `ThreadPoolExecutor` fire-and-forget (flag `TRAINING_TRIGGER_ENABLED`). Fix G5 (`EnsembleRegistry.get()` re-chequea disco cuando cache=`None`) y model_version leído del bundle. **C5 cerrado**: `retrain_service.retrain_station` delega en `training_service.train_station(source="retrain")`. Endpoint `GET /health-monitor/training-state` + widget `EquiposEnWarmup` en dashboard (coord/admin). Provenance G7 en `theta_*.json`: `model_version`, `rows_train`, `median_hsp`, `trained_at`, `training_source`. Docs: `docs/spec-auto-training-onboarding.md`, `docs/plan-auto-training-onboarding.md`. Ingest: `scripts/talara_ingest.py` para el backfill de `services/ml-proposal/dataset/CA-TA-01_TALARA_DATASET.csv` (52k rows, 181d). Tests: 43 nuevos backend + 6 frontend + 6 script (ml=168, gw/iot/ops sin regresión). E2E docker con dataset TALARA pendiente — runbook en `docs/onboarding-talara-log.md`. |
...
| I2 | **Gestión de Problemas** | ✅ HECHO + recurrencia (2026-07-05) | Tabla `problemas` (migr ops_006) + `incidencias.problema_id`; CRUD + vincular; endpoints via gateway. **Detección de recurrencia (visibilidad/proactividad)**: `GET /problemas/reincidentes` (≥3 correctivas/90d por equipo, configurable, excluye equipos con problema abierto) + `GET /problemas/resumen`; widget `EquiposReincidentes` en dashboard con botón "Crear problema" (pre-llenado + vincula incidencias, SUGIERE no crea). Valor de negocio documentado en spec-itil §Adenda. Tests ops TestReincidentes (5) + e2e. |
