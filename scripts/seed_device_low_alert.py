"""
Seed stable readings for any device to produce alerta BAJA (RUL >= 60 dias).
...
Usage:
    python scripts/seed_device_low_alert.py DEVICE_ID [--url URL] [--db DB_URL]

Example:
    python scripts/seed_device_low_alert.py T109
    python scripts/seed_device_low_alert.py T109 --url http://localhost:8001/api/v1/iot/readings
"""
...
def main():
    parser = argparse.ArgumentParser(description="Seed stable readings for a device (alerta baja)")
    ...
    rng = random.Random(hash(device + "_low") & 0xFFFFFFFF)
