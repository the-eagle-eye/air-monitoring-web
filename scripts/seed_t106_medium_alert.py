"""
Seed moderately degraded readings for T106 to produce alerta MEDIA (RUL 30-59 dias).
...
Usage:
    python scripts/seed_t106_medium_alert.py [--url URL]
"""
...
N_READINGS = 300
# Moderate degradation: progress from 0.40 to 0.60
PROGRESS_START = 0.60
PROGRESS_END = 0.75


def main():
    parser = argparse.ArgumentParser(description="Seed moderately degraded readings for T106 (alerta media)")
    parser.add_argument(
        "--url",
    ...
    for i in range(N_READINGS):
        ...
        for field, (low, high) in HEALTHY_BASELINES.items():
            ...
            # Moderate degradation — same patterns as high profile but at 40-60% intensity
