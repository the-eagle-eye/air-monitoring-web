"""
Seed critically degraded readings for T107 to produce alerta ALTA (RUL < 30 dias).

Generates 300 readings with severe sensor degradation — equipment near failure.
Uses higher degradation intensity than T102 to ensure RUL < 30 threshold is crossed.
...
Usage:
    python scripts/seed_t107_high_alert.py [--url URL]
"""
...
def main():
    parser = argparse.ArgumentParser(description="Seed critically degraded readings for T107 (alerta alta)")
    parser.add_argument(
        "--url",
