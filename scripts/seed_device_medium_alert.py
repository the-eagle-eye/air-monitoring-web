"""
Seed moderately degraded readings for any device to produce alerta MEDIA (RUL 30-59 dias).

Generates 300 readings with partial sensor degradation — equipment shows early wear
but has not reached critical threshold. Prediction must be triggered manually from the app.

Expected result after prediction:
  - risk_level: media
  - 30 <= RUL < 60 dias
  - failure_probability ~0.35-0.60

Usage:
    python scripts/seed_device_medium_alert.py DEVICE_ID [--url URL] [--db DB_URL]

Example:
    python scripts/seed_device_medium_alert.py T109
    python scripts/seed_device_medium_alert.py T109 --url http://localhost:8001/api/v1/iot/readings
"""
...
def main():
    parser = argparse.ArgumentParser(description="Seed moderately degraded readings for a device (alerta media)")
    parser.add_argument("device", help="Device ID (e.g. T109)")
    parser.add_argument(
        "--url",
    ...
    rng = random.Random(hash(device + "_medium") & 0xFFFFFFFF)

    print(f"\n--- Seeding {N_READINGS} moderately degraded readings for {device} (alerta MEDIA target: RUL 30-59) ---")
    ok = 0
    err = 0
    ...
    for i in range(N_READINGS):
        ...
        for field, (low, high) in HEALTHY_BASELINES.items():
            ...
            baseline = (low + high) / 2
            noise_std = (high - low) * 0.05 if (high - low) > 0 else 0.01
    ...
    print("Expected: risk_level=media, RUL 30-59 dias, failure_probability ~0.35-0.60")
