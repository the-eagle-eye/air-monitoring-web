"""
Seed critically degraded readings for any device to produce alerta ALTA (RUL < 30 dias).
...
Usage:
    python scripts/seed_device_high_alert.py DEVICE_ID [--url URL] [--db DB_URL]

Example:
    python scripts/seed_device_high_alert.py T109
    python scripts/seed_device_high_alert.py T109 --url http://localhost:8001/api/v1/iot/readings
"""
...
def main():
    parser = argparse.ArgumentParser(description="Seed critically degraded readings for a device (alerta alta)")
    ...
    rng = random.Random(hash(device + "_high") & 0xFFFFFFFF)
