"""
Seed stable readings for T105 to produce alerta BAJA (RUL >= 60 dias).
...
Usage:
    python scripts/seed_t105_low_alert.py [--url URL]
"""
...
def main():
    parser = argparse.ArgumentParser(description="Seed stable readings for T105 (alerta baja)")
    parser.add_argument(
        "--url",
    ...
    for i in range(N_READINGS):
        ...
        for field, (low, high) in HEALTHY_VALUES.items():
            if low == high:
                ...
                # Very tight gaussian around center — equipment in excellent condition
                center = (low + high) / 2
                std = (high - low) * 0.10
