...
type EditFormProps = {
  ...
  onCausaRaizChange: (v: string) => void;
  onSubmit: (e: React.FormEvent) => void;
  onCancel: () => void;
};

function ProblemaEditForm({
  estado,
  descripcion,
  causaRaiz,
  saving,
...
function ProblemaDetailsCard({ problema }: { problema: Problema }) {
  return (
...
export default function ProblemaDetailPage() {
  ...
  if (!problema) {
    return <div className="mx-auto max-w-5xl px-4 py-8">{backLink}</div>;
  }
    ...
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      {editing ? (
        <ProblemaEditForm
          ...
          onCausaRaizChange={setCausaRaiz}
          onSubmit={handleSave}
          onCancel={() => setEditing(false)}
        />
      ) : (
        <ProblemaDetailsCard problema={problema} />
      )}
