...
import { GW, IOT, ML, EMPTY_LIST } from './helpers/constants';

const AUTH_LOGIN = `${GW}/api/v1/auth/login`;
const AUTH_ME = `${GW}/api/v1/auth/me`;

const ADMIN_USER = { id: 1, email: 'admin@oefa.gob.pe', nombre: 'Admin', apellido: 'OEFA', rol: 'administrador' };
const TECNICO_USER = { id: 2, email: 'tecnico1@oefa.gob.pe', nombre: 'Tecnico', apellido: 'OEFA', rol: 'tecnico' };
...
test.describe('Login flow', () => {
  test('renders login form with credential hints', async ({ page }) => {
    await page.route(AUTH_ME, (route) => route.fulfill({ status: 401, body: '{}' }));
  ...
  test('shows error on wrong credentials', async ({ page }) => {
    ...
    await page.route(AUTH_LOGIN, (route) =>
      route.fulfill({ status: 401, contentType: 'application/json', body: JSON.stringify({ detail: 'Credenciales incorrectas' }) }),
    );
    await page.goto('/login');
  ...
  test('shows loading state while submitting', async ({ page }) => {
    ...
    await page.route(AUTH_LOGIN, async (route) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({ access_token: 'tok', refresh_token: 'ref', usuario: ADMIN_USER }),
      });
    });
    await page.goto('/login');
    ...
    await expect(page.getByRole('button', { name: /Ingresando.../i })).toBeVisible({ timeout: 5_000 });
  });
  ...
  test('admin login redirects to /dashboard', async ({ page }) => {
    await page.route(AUTH_LOGIN, (route) =>
      route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ access_token: 'mock-access-token', refresh_token: 'mock-refresh-token', usuario: ADMIN_USER }),
      }),
    );
  ...
  test('tecnico login redirects to /dashboard-tecnico', async ({ page }) => {
    ...
    await page.route(AUTH_LOGIN, (route) =>
      route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ access_token: 'mock-token-tecnico', refresh_token: 'mock-refresh-tecnico', usuario: TECNICO_USER }),
      }),
    );
    await page.route(AUTH_ME, (route) =>
      route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(TECNICO_USER) }),
    );
    await page.goto('/login');
